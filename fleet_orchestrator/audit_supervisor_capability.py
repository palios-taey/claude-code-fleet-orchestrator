...
Authority model (OS-separated principals):
  - Issuance identity is SO_PEERCRED uid mapped by ORCH_AUDIT_CAPABILITY_UID_MAP.
    tmux session names and ORCH_SESSION_ID environ are not authority.
  - Issuer (systemd User=orch-cap) alone holds the Ed25519 private key (0600).
  - API verifier loads only the public key and cannot mint.
  - Shared-UID workers (typically uid 1000) are not in the uid map and cannot
    read the orch-cap-owned private key.
...
def set_peer_session_override(_session_id: Optional[str] = None) -> None:
    raise AuditContractError(
        "peer session override/env identity is not an authority channel; "
        "issuer tests inject a pid→tmux-session resolver, never ORCH_SESSION_ID"
    )
...
# Distinct-UID deploy path. Never default under $HOME/XDG — that is the shared
# mira uid data dir and lets workers read a 0600 key they own.
...
def default_key_paths(env: Optional[Mapping[str, str]] = None) -> Dict[str, Path]:
    ...
    key_dir = Path(str(values.get("ORCH_AUDIT_CAPABILITY_KEY_DIR") or DEFAULT_CAPABILITY_DIR))
    return {
        "private": Path(
