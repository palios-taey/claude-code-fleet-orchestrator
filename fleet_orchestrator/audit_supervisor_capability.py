...
Authority model (same-UID topology safe):
  - Issuance identity comes from the connecting process (SO_PEERCRED pid → tmux pane
    ownership), never from JSON body.from or ORCH_SESSION_ID environ.
  ...
  - Worker connecting to the issuer is attested as its tmux session and fails the
    project-supervisor match; forging ORCH_SESSION_ID cannot mint for the supervisor.
