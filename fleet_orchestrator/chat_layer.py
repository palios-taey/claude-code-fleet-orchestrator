...
async def chat_post(lineage: str, req: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        bound_question_reply: Optional[Dict[str, Any]] = None
        bound_hold_reply_task_id = ""
        auto_cleared_holds: Optional[Dict[str, Any]] = None
        auto_answered_questions: Optional[Dict[str, Any]] = None
        if reply_to_question_id:
            ...
            if not bound_gate_reply.get("ok"):
                from fleet_orchestrator.orch_schema import (
                    auto_answer_open_questions_for_reply,
                    get_supervisor_human_review_holds,
                    resolve_task_id,
                )

                bound_question_reply = auto_answer_open_questions_for_reply(
                    lineage,
                    answer=str(text or ""),
                    answered_by=str(sender or "operator"),
                    reply_to_question_id=reply_to_question_id,
                    config=cfg,
                )
                target_was_purged = reply_to_question_id in set(bound_question_reply.get("purged_question_ids") or [])
                if bound_question_reply.get("answered_count") or target_was_purged:
                    bound_gate_reply = None
                    metadata = {
                        **metadata,
                        "reply_to_question_id": reply_to_question_id,
                        "open_question_reply": True,
                        "auto_answered_question": bool(bound_question_reply.get("answered_count")),
                        "purged_stale_question": target_was_purged,
                    }
                else:
        ...
        if message["role"] == "user":
            if bound_hold_reply_task_id:
            ...
            elif bound_question_reply:
                auto_answered_questions = bound_question_reply
                task_ids = [
                    str(item.get("task_id") or "").strip()
                    for item in (bound_question_reply.get("answered_questions") or [])
                ...
                if task_ids:
                    ...
                    for task_id in dict.fromkeys(task_ids):
                        result = auto_clear_human_review_holds_for_reply(
                            lineage,
                            responded_by=str(sender or "operator"),
                            reply_to_question_id=task_id,
                            config=cfg,
                        )
                        cleared.extend(result.get("cleared_holds") or [])
                    if cleared:
                        auto_cleared_holds = {
                            "ok": True,
                            "supervisor": _normalize_lineage(lineage),
                            "cleared_count": len(cleared),
                            "cleared_holds": cleared,
                            "reply_to_question_id": reply_to_question_id,
                        }
            elif not reply_to_question_id:
                from fleet_orchestrator.orch_schema import (
                    auto_answer_open_questions_for_reply,
                    auto_clear_human_review_holds_for_reply,
                )
                ...
                auto_answered_questions = auto_answer_open_questions_for_reply(
                    lineage,
                    answer=str(text or ""),
                    answered_by=str(sender or "operator"),
                    config=cfg,
        ...
        if auto_answered_questions and (
            auto_answered_questions.get("answered_count")
            or auto_answered_questions.get("purged_question_ids")
        ):
            response["auto_answered_open_questions"] = auto_answered_questions
        return response
