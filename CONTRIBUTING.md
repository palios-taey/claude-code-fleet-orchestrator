...
- Agent-facing error, wake, API, and CLI surfaces are registry-gated. Run `python scripts/verify-ai-native-coherence.py` and `python tests/ai_native_coherence_acceptance.py` before submitting a PR that changes those surfaces; new `needs-fix` rows are not allowed without explicit reviewed baseline movement.
