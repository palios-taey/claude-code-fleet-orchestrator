...
that conductor's ``recurring_trigger_runner.py`` implements. Backward
compatible with the existing JSON schema; adds one optional field
...
def should_fire(trig: dict, now_local: datetime) -> bool:
    """Exact-minute match within a listed hour. Same rule as conductor's
    recurring_trigger_runner.py; preserved for backward compat with the
...
def fire_trigger(r, trig: dict, now_local: datetime, dry_run: bool = False) -> str:
    ...
    dedup_key = f"taey:orch-cron-fired:{fire_id}"
    if r is not None:
...
def append_state_file(state_file: Optional[str], record: dict) -> Optional[str]:
    ...
    storing the full log in the graph (Logos, Phase C consultation
    2026-05-26). <1ms IO cost per fire, no cardinality explosion at
