...
def _stop_gate_dedup(r, node_id: str, current_task_id: str, decision_key: str,
    ...
    try:
        return bool(r.set(
            f"taey:orch-stop-gate:{node_id}:{current_task_id}:{decision_key}",
            "1",
            nx=True,
            ex=ttl_sec,
        ))
...
def _evaluate_user_stop_conditions(r, node_id: str, task_state: dict,
    ...
    for condition in stop_conditions:
        if condition == "stop_when_all_ready_tasks_dispatched":
            ...
            if r.get(f"taey:{node_id}:awaiting_user_decision"):
                return condition, next_ready
...
def notify_supervisor_of_stuck(r, supervisor: str, node_id: str,
    ...
    Audit fix (Logos contract #3 + Gaia dispatch #2, code audit
    2026-05-26): dedup is keyed per ``(node_id, task_id)`` not just
    ...
    """
    task_id = task.get("task_id", "?")
    dedup_key = f"taey:orch-watch-stuck:{node_id}:{task_id}"
...
def handle_done_del(r, node_id: str, snapshot: dict,
    ...
    Audit fix (Gaia orch-watch #2, code audit 2026-05-26): the Stop hook
    ...
    done_marker = r.get(f"taey:{node_id}:last_clear_was_done")
    if not done_marker:
    ...
    r.delete(f"taey:{node_id}:last_clear_was_done")
...
def _redis_now(r) -> float:
    ...
    timestamps written by other nodes / processes (Gaia orch-watch #3,
    code audit 2026-05-26: client-side clock skew across multi-host
...
def investigate(r, node_id: str, event_type: str,
    ...
    Audit fix (Gaia orch-watch #1, code audit 2026-05-26): skip snapshot
    ...
    if last_outcome and last_outcome.get("outcome") == "done":
        # Grace window (treasurer system-integration sign-off TIER-3,
        # noise complaint): the Stop hook's CAS done-clear is async with
        ...
        try:
            la = float(r.get(f"taey:{node_id}:last_activity") or 0)
        except Exception:
            la = 0
    ...
    # Audit fix #3 (Gaia, code audit 2026-05-26) + fix-of-fix (real-fleet
    # observation 2026-05-26 evening): a task cannot be "stuck" longer
    ...
    last_activity_raw = r.get(f"taey:{node_id}:last_activity")
    try:
...
def main():
    ...
    # Audit fix (Logos critical reliability gap, code audit 2026-05-26):
    ...
    # poll backstop" design from the Phase B consultation. The fix is to
