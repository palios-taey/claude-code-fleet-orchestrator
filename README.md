...
| `src/fleet_orchestrator/config.py` | `OrchConfig` + Redis/Neo4j connection helpers; path-flexible `.env` loading. `ORCH_NEO4J_URI` is required, and auth-required deployments must set both `ORCH_NEO4J_USER` and `ORCH_NEO4J_PASS`. Use `ORCH_NEO4J_NOAUTH=1` only for explicit no-auth Neo4j deployments. |
