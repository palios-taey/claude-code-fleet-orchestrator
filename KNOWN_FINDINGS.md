...
| ENG-NEO4J-AUTH-SCHEME | HIGH | OPEN | Stop-hook outcome-recording crashes with neo4j AuthError "Unsupported authentication token, missing key `scheme`" on task completion (taeys-hands, p0-systemd-launch). **Observed:** live config.py:193 uses `auth=(user,pass)` tuple; that path RETURNs 1 fine from conductor's interactive env; neo4j driver 5.28.2 uniform across all venvs — so NOT a tuple-vs-dict bug and NOT a version split (both taeys-hands hypotheses refuted by direct check). Task DID complete; crash is post-completion in the outcome-recording hook only. **Inferred:** the Stop hook runs as a subprocess inheriting Claude Code's env (not a shell's); if NEO4J creds resolve empty there, auth becomes a malformed token → this exact error. Compounding: config does NOT fail-loud on empty creds (it builds the broken token instead) — ties to ws1-config fail-loud. **Unknown:** not yet confirmed the hook subprocess env is the actual delta (needs in-hook cred-length probe, taeys-hands' substrate). Fix folds into ws1-config (fail loud + explicit basic_auth) + hook env sourcing. |
