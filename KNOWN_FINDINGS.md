...
| ENG-LIVE-BIND-DRIFT | HIGH | FIXED@85f352a (the-conductor) + production-verified 2026-06-01 | Live tasks-api `:5002` bound `0.0.0.0` (LAN-routable, no auth) despite the shipped launcher being correct. ROOT CAUSE: the running process was started by an OLD peer-respawn.sh (--host 0.0.0.0) and never restarted; the cron (`* * * * * peer-respawn.sh`, idempotent) no-ops while the `tasks-api` tmux session exists, so the loopback fix never took effect on the live process. FIX: committed launcher 0.0.0.0->127.0.0.1 (85f352a) + `tmux kill-session -t tasks-api` so cron recreates it on loopback. VERIFIED: 127.0.0.1:5002->HTTP200, 127.0.0.1:5002->HTTP000 (LAN refused). Closes ws3-localbind. LESSON: a shipped launcher fix is not live until the long-lived daemon is cycled — verify with `ss -ltnp`, never assume. |
| V1-SEND-ATTACH-RACE | MEDIUM (fleet-notify) | OPEN (routed to taeys-hands peer) | 2026-06-01: ChatGPT AT-SPI send raced a ~48KB attachment — send fired before upload finished, 3-attempt/~7s retry window expired -> send_failed; a manual click seconds later submitted instantly. PRECISE FIX (taeys-hands-diagnosed): gate ChatGPT send on attachment-ready (poll upload-complete / send-enabled) before Return, OR widen the attachment send-retry window. Specific instance of V1-SEND-BUG for large-attachment dispatch. Owner = taeys-hands repo/peer (engineering-owner-runs-their-project); folded into ws6-fleet-notify track. |
