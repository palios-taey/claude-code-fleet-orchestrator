...
- Closed Gaia's pending v0.4.1 zero-dependency wake gap from `/path/to/repo`: `lib.orch_schema.create_task()` now carries owner metadata in the initial write and immediately wakes an idle owner when the new task has no upstream dependencies.
