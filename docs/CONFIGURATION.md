...
- **`ORCH_HOST` defaults to `127.0.0.1`** (loopback/private). If you bind a
  ...
  untrusted callers.
- The completion-evidence check first enforces shape/plausibility, then records a separate truth marker. `completion_evidence_verification.status=VERIFIED` only when `gh api` confirms the GitHub commit exists in a repo that is present in `ORCH_COMPLETION_ALLOWED_REPOS` and every required independent gate context passed for that exact `commit_sha` from trusted GitHub actors/apps. If `evidence.repo` is omitted, the configured or inferred runtime repo is used, but it must still be present in `ORCH_COMPLETION_ALLOWED_REPOS`. Otherwise the completed task is surfaced as `UNVERIFIED`. `repo` is optional context only and cannot satisfy completion evidence by itself; off-allowlist repos stay `UNVERIFIED`, not rejected. Local/non-repo completions and completions without `commit_sha` stay completed but explicitly unverified. The token is still the control for *who can reach the port*.
...
| `ORCH_COMPLETION_GITHUB_REPO` / `GITHUB_REPOSITORY` | inferred from `gh api repos/:owner/:repo` | Fallback GitHub `OWNER/REPO` used to verify completed-task `commit_sha` evidence when the evidence omits `repo`. The fallback repo must still be present in `ORCH_COMPLETION_ALLOWED_REPOS`; if no repo or `gh` access is available, completions are marked `UNVERIFIED`, not rejected. |
| `ORCH_COMPLETION_ALLOWED_REPOS` | unset | Comma-separated GitHub `OWNER/REPO` allowlist for completed-task verification. When unset or empty, startup logs a warning and all commit-based completions stay `UNVERIFIED` until configured. A caller-supplied or inferred repo outside this list is `UNVERIFIED`, even if the commit and statuses exist. |
