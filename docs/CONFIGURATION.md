...
| `ORCH_OUTWARD_SESSION` | unset | Worker session id used by GitHub outward capability when `--session` is omitted. Falls back to `TAEY_SESSION`, then `TMUX_SESSION`, then `tmux display-message -p #S`. Empty session fail-closes mutating GitHub argv. |
