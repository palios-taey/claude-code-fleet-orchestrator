...
| `ORCH_OUTWARD_HANDLE` | unset | Revocable possession handle minted in github-brokerd process memory (never Redis/`current_task`). Worker GitHub clients present this handle; a claimed session name cannot substitute for it. |
...
| `ORCH_GITHUB_BROKER_SOCKET` | unset | Unix socket path for the GitHub outward broker. Worker `gh` clients send argv here; they never receive `GH_TOKEN`. Required for `scripts/gh-outward`. |
