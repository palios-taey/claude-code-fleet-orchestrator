...
| `ORCH_GITHUB_BROKER_CONTROL_SESSIONS` | `ORCH_SESSION_IDS` | Comma-separated supervisor tmux sessions allowed to mint/revoke. Worker `*-grok`/`*-gemini`/`*-claude` seats cannot mint even when they share uid 1000 and can open the control socket. |
