...
| `ORCH_GITHUB_BROKER_CONTROL_SESSIONS` | `ORCH_SESSION_IDS` | Comma-separated supervisor tmux sessions allowed to mint/revoke from a TTY. The systemd API daemon (`uvicorn` + `tasks_api`) may mint with no TTY. Worker seats cannot mint. |
