...
| `ORCH_COMPLETION_GITHUB_REPO` / `GITHUB_REPOSITORY` | inferred from `gh api repos/:owner/:repo` | Runtime GitHub repo used by PR-reference reconciliation when a task names a bare PR number, and used to classify no-`commit_sha` completed writes before any caller-supplied `evidence.repo`. If the runtime repo is allowlisted as `:gated`, observation-only completed writes are rejected unless valid non-production `supervisor_verification` is present; commit evidence still must include explicit `completion_evidence.repo` and is never inferred. |
