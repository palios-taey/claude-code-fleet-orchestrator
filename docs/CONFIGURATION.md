...
| `ORCH_COMPLETION_GITHUB_REPO` / `GITHUB_REPOSITORY` | inferred from `gh api repos/:owner/:repo` | Runtime GitHub repo used only by PR-reference reconciliation when a task names a bare PR number. It is not used to fill missing `completion_evidence.repo` for commit evidence; commit evidence without `repo` is rejected. |
