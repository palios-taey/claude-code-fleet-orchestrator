...
| `ORCH_GITHUB_BROKER_CONTROL_CGROUP` | `/system.slice/fleet-orchestrator-api.service` | Kernel cgroup of the system API unit allowed to mint/revoke. Same-UID workers in `user.slice`/`tmux-spawn` cannot join this cgroup. Cmdline is not identity. |
