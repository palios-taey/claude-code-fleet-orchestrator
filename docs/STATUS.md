...
- Live verification is recorded in `PHASE_V106_NEO4J_AUTH_VERIFICATION.md`, including the current Mira host-state drift where `10.0.0.163:7689` is no-auth while `localhost:7687` is the auth-required endpoint that reproduced the original `Unsupported authentication token, missing key scheme` failure.
...
- PATCH `/api/task/{id}` now preserves the existing task owner when the caller omits `owner`, instead of blanking it on every status update.
