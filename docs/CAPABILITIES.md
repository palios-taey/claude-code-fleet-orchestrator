...
| Gate template | Flag-gated off by default. `ORCH_GATE_TEMPLATE_ENABLED=1` applies the scout/code/audit/verify/family gate scaffold during plan ingest. | `fleet_orchestrator.orch_template.py`; `tests/orch_template_acceptance.py`; ingest a plan with the flag on and inspect created gate tasks. |
