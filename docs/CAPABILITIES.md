...
| Refs | Live but fail-safe gated by `ORCH_REF_ALLOWED_ROOT`. Plan refs are stored as structured metadata and rendered as file-line pointers when allowed. | Add `[ref: path]` or `[ref: path:Lx-Ly]` to a plan, set `ORCH_REF_ALLOWED_ROOT`, then inspect `GET /api/projects/{id}` ref_context. |
| Wake packet | Live by default. `ORCH_WAKE_PACKET_ENDPOINT_ENABLED=0` disables only the provenance-bound packet endpoint; deprecated alias `ORCH_WAKE_PACKET_ENABLED` is still honored. Empty ref tiers mean no refs were attached or selected for that tier; the assembler renders attached refs when present. | `GET /api/sessions/{session}/wake-packet?cli=claude`; `docs/DYNAMIC_CONTEXT_REFS.md`; `tests/wake_packet_acceptance.py`; packet fields `packet_id` and `provenance_hash`. |
| Decision receipts | Live by default. `ORCH_DECISION_RECEIPTS_ENABLED=0` disables best-effort content-hashed receipt writes to Redis stream `orch:streams:decision_receipts`. No consumer is wired in this phase. | `fleet_orchestrator.decision_receipt.RECEIPT_STREAM`; `tests/decision_receipt_acceptance.py`; `redis-cli XREVRANGE orch:streams:decision_receipts + - COUNT 5`. |
...
| Loop engine | Live by default. `ORCH_LOOPS_ENABLED=0` disables the additive loop declaration/advance API routes backed by `fleet_orchestrator.loop_engine`; no `orch-loop` CLI or core dispatch integration is shipped on main. | `POST /api/loops/declare`; `POST /api/loops/{loop_id}/advance`; `GET /api/loops/{loop_id}/should-stop`; `tests/loop_engine_acceptance.py`. |
...
| Trace | Split. Orchestrator emits default-on decision receipts; notification delivery trace lives in notify Redis stream `taey:notify_trace`. | Orchestrator: `ORCH_DECISION_RECEIPTS_ENABLED=0` disables receipt emission; notify: `taey-trace` and `notifications/trace.py`. |
