...
- `recurring: true` marks a completed task as re-claimable by `dispatch()` for the next cycle. This is for markdown-tracked repeated work items, not the cron-factory `kind=recurring` reservation in `docs/SCHEMA.md`. A scheduled `orch-cron` registry entry with `task_id` can drive that cadence by dispatching the same recurring task id when its wall-clock trigger fires; a registry entry with `project` dispatches the project's current next-ready step without resetting any task state, then re-claims completed recurring steps by lowest reclaim count after the pending dependency chain drains. The task still completes through the normal evidence-gated task API before the next re-claim.
